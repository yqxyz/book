...
<script>
    ...
    $(function ($) {
        $('#footable').footable({
            "columns": [
                {"name": "id", "title": "ID", "filterable": false},
                {"name": "name", "title": "分类名"},
                {
                    "name": "id", "title": "操作",
                    "formatter": function (value, options, rowData) {
                        return "<button  class='btn-delete btn btn-primary btn-xs' itemid='" + rowData.id + "'>删除</button>";
                    }
                }
            ],
            "rows": $.get("/itemCat/list")
        });
        ...
        $('#confirm').click(function () {
            $('#myModal6').modal('hide');
            $.post("/itemCat/del", {id: itemId}, function (data) {
                if (data.status == 200) {
                    a = 1;
                    toastr.options = {
