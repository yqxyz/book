    ...
    $(function ($) {
        ...
        $('#footable').footable({
            "columns": [
                ...
                {"name": "num", "title": "num"},
                {"name": "auther", "title": "auther"},
                {"name": "itemdesc", "title": "itemdesc"},
                {
                    "name": "status", "title": "Status",
                    "formatter": function (value, options, rowData) {
                        if (rowData.status == 1)
                            return "<span class=\"btn btn-primary btn-xs\">上架</span>";
                        else
                            return "<span class=\"btn btn-warning btn-xs\">下架</span>";
                    }
                },
                {
                    "name": "操作", "title": "操作",
                    "formatter": function (value, options, rowData) {
                        return "<a href='/item/edit/" + rowData.itemId + "'><span class=\"btn-white btn btn-xs\">编辑</span></a>" +
                            "<a><span id='delBtn' itemid='" + rowData.itemId + "' class=\"btn-white btn btn-xs\">删除</span></a>";
                    }
                }
            ],
            "rows": $.get("/item/list")
        ...
        $('#confirm').click(function () {
            $('#myModal6').modal('hide');
            $.post("/item/del", {id: id}, function (data) {
                if (data.status == 200) {
                    a = 1;
                    toastr.options = {
                    ...
                    toastr.options.onHidden = function () {
                        window.location.href = "${pageContext.request.contextPath}/admin/itemList.html"
                    };
